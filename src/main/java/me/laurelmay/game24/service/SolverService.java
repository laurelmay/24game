...
public class SolverService {
  ...
  private Optional<Operation> toOuterPairOperations(List<Integer> numbers, List<Class<? extends Operation>> operations) {
    try {
      Operand.Expression lhs = new Operand.Expression(
        findConstructor(operations.get(0)).newInstance(new Operand.Number(numbers.get(0)),
          new Operand.Number(numbers.get(1))));
      Operand.Expression rhs = new Operand.Expression(
  ...
  private Optional<Operation> toLeftToRightOperations(List<Integer> numbers, List<Class<? extends Operation>> operations) {
    try {
      Operand a = new Operand.Expression(
        findConstructor(operations.get(0)).newInstance(new Operand.Number(numbers.get(0)),
          new Operand.Number(numbers.get(1))));
  ...
  private Optional<Operation> toLeftToRightOperationsGroupingSecondAndThird(List<Integer> numbers, List<Class<? extends Operation>> operations) {
    try {
      Operand inner = new Operand.Expression(
        findConstructor(operations.get(1)).newInstance(new Operand.Number(numbers.get(1)),
          new Operand.Number(numbers.get(2))));
